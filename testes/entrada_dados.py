...
window.wm_title('Grafos - Prof.Raimir - Equipe05 (Marcos Aires, Fernando Oliveira e Ricardo Carubbi)')

# Create label for window window
label1 = tk.Label(window, text = "Caixeiro Viajante - TSP", font=('calibre',12, 'bold'))
# label1.pack()

label2 = tk.Label(window, text ='Posição das Cidades', font = "50")
...
def open_Toplevel2():
	...
	top2.title("Desempenho")
	
	# specify size
	top2.geometry("200x100")
	
	# Create label
	label = tk.Label(top2, text = "Dados de duração da tarefa")
	
	# Create exit button.
	button = tk.Button(top2, text = "Fechar janela", command = top2.destroy)

	# Posicionando rótulo e botão na tela
...
Button1 = tk.Checkbutton(window, text = "Aleatória",
...
Button2 = tk.Checkbutton(window, text = "Definida",
...
frame1 = tk.Frame(window, height = 800, width = 600, bg="white", bd = 4)
...
btnOpenImage = tk.Button(window, text ='Abrir imagem', command = open_img)
...
def submit():
    ...
    cities.append(City(name=name, pos=(int(lon), int(lat)), number=ordem))
...
title_label = tk.Label(window, text = 'Cidades:', font=('calibre',12, 'bold'))
	
# creating a label for ordem_cities using widget Label
ordem_label = tk.Label(window, text = 'Ordem', font=('calibre',10, 'bold'))

# creating a entry for input city name using widget Entry
ordem_entry = tk.Entry(window,textvariable = ordem_var, font=('calibre',10,'normal'))

# creating a label for name using widget Label
name_label = tk.Label(window, text = 'Cidade', font=('calibre',10, 'bold'))
...
lon_label = tk.Label(window, text = 'Longitude', font = ('calibre',10,'bold'))

# entrando com as coordenadas
lon_entry =tk.Entry(window, textvariable = lon_var, font = ('calibre',10,'normal'))

# creating a label for password
lat_label = tk.Label(window, text = 'Latitude', font = ('calibre',10,'bold'))

# entrando com as coordenadas
lat_entry =tk.Entry(window, textvariable = lat_var, font = ('calibre',10,'normal'))
...
# passw_entry=tk.Entry(window, textvariable = passw_var, font = ('calibre',10,'normal'), show = '*')


# creating a button using the widget Button that will call the submit function
sub_btn=tk.Button(window,text = 'Salvar Dados', command = submit)
