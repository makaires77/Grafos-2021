...
def strategy_largest_first(G, cores):
    """Returns a list of the nodes of ``G`` in decreasing order by
    degree.

    ``G`` is a NetworkX graph. ``cores`` is ignored.
...
def strategy_random_sequential(G, cores):
...
def strategy_smallest_last(G, cores):
    ...
    for _ in G:
        ...
        # Update degrees of removed node's vizinhos.
        for v in H[u]:
            degree = H.degree(v)
        ...
        H.remove_node(u)
        lbound = min_degree - 1  # Subtract 1 in case of tied vizinhos.
...
def strategy_independent_set(G, cores):
    """Uses a greedy independent set removal strategy to determine the
    cores.

    This function updates ``cores`` **in-place** and return ``None``,
...
def strategy_connected_sequential_bfs(G, cores):
    """Returns an iterable over nodes in ``G`` in the order given by a
    breadth-first traversal.

    The generated sequence has the property that for each node except
    the first, at least one vizinho appeared earlier in the sequence.

    ``G`` is a NetworkX graph. ``cores`` is ignored.

    """
    return strategy_connected_sequential(G, cores, 'bfs')



def strategy_connected_sequential_dfs(G, cores):
    """Returns an iterable over nodes in ``G`` in the order given by a
    ...
    """
    return strategy_connected_sequential(G, cores, 'dfs')



def strategy_connected_sequential(G, cores, traversal='bfs'):
    """Returns an iterable over nodes in ``G`` in the order given by a
...
def strategy_saturation_largest_first(G, cores):
    """Iterates over all the nodes of ``G`` in "saturation order" (also
    known as "DSATUR").

    ``G`` is a NetworkX graph. ``cores`` is a dictionary mapping nodes of
    ``G`` to cores, for those nodes that have already been colored.

    """
    distinct_cores = {v: set() for v in G}
    for i in range(len(G)):
        # On the first time through, simply choose the node of highest degree.
        if i == 0:
            node = max(G, key=G.degree)
            yield node
            # Add the color 0 to the distinct cores set for each
            # vizinhos of that node.
            for v in G[node]:
                distinct_cores[v].add(0)
        else:
            # Compute the maximum saturation and the set of nodes that
            # achieve that saturation.
            saturation = {v: len(c) for v, c in distinct_cores.items()
                          if v not in cores}
            # Yield the node with the highest saturation, and break ties by
            # degree.
            node = max(saturation, key=lambda v: (saturation[v], G.degree(v)))
            yield node
            # Update the distinct color sets for the vizinhos.
            color = cores[node]
            for v in G[node]:
                distinct_cores[v].add(color)
...
def greedy_color(G, strategy='largest_first', interchange=False):
    ...
    Attempts to color a graph using as few cores as possible, where no
    ...
    strategy : string or function(G, cores)
       A function (or a string representing a function) that provides
       the coloring strategy, by returning nodes in the ordering they
       should be colored. ``G`` is the graph, and ``cores`` is a
       dictionary of the currently assigned cores, keyed by nodes. The
       function must return an iterable over all the nodes in ``G``.

       If the strategy function is an iterator generator (that is, a
       function with ``yield`` statements), keep in mind that the
       ``cores`` dictionary will be updated after each ``yield``, since
       this function chooses cores greedily.
    ...
    if interchange:
        ...
        if strategy is strategy_saturation_largest_first:
            ...
            raise nx.NetworkXPointlessConcept(msg)
    cores = {}
    nodes = strategy(G, cores)
    if interchange:
        return _interchange.greedy_coloring_with_interchange(G, nodes)
    for u in nodes:
        # Set to keep track of cores of neighbours
        neighbour_cores = {cores[v] for v in G[u] if v in cores}
        # Find the first unused color.
        for color in itertools.count():
            if color not in neighbour_cores:
                break
